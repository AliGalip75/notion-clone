...
import type { Block, BulletedListBlockData, BlockType } from "@/types";
...
export default function BulletedListBlock({ block, onEnter, autoFocus, onDelete }: BulletedListBlockProps) {
  const queryClient = useQueryClient();
  const data = block.data as unknown as BulletedListBlockData;
  ...
  const transformBlock = useMutation({
    mutationFn: (type: BlockType) => {
      ...
      switch (type) {
        ...
        case "bulleted_list": defaultData = { content: newContentText }; break;
        case "numbered_list": defaultData = { content: newContentText }; break;
  ...
  return (
    <div className="py-1 group relative flex items-start gap-2">
      <div className="w-4 h-6 flex items-center justify-center shrink-0 mt-0.5">
        <div className="w-1.5 h-1.5 rounded-full bg-[var(--color-text)]" />
      </div>
