...
import type { Block, NumberedListBlockData, BlockType } from "@/types";
...
export default function NumberedListBlock({ block, onEnter, autoFocus, onDelete }: NumberedListBlockProps) {
  const queryClient = useQueryClient();
  const data = block.data as unknown as NumberedListBlockData;
  ...
  return (
    <div className="py-1 group relative flex items-start gap-2">
      <div className="w-6 h-6 flex items-center justify-end shrink-0 text-sm mt-0.5" style={{ color: "var(--color-text)" }}>
        1.
      </div>
